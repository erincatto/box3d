...
void CreateJointGrid( b3WorldId worldId )
{
	...
	for ( int k = 0; k < n; ++k )
	{
		for ( int i = 0; i < n; ++i )
		{
			...
			bodyDef.position = b3ToPos( (b3Vec3){ fk, -fi, 0.0f } );
...
void CreateLargePyramid( b3WorldId worldId )
{
	...
	{
		b3BodyDef bodyDef = b3DefaultBodyDef();
		bodyDef.position = b3ToPos( (b3Vec3){ 0.0f, -1.0f, 0.0f } );
	...
	for ( int i = 0; i < baseCount; ++i )
	{
		...
		for ( int j = i; j < baseCount; ++j )
		{
			...
			bodyDef.position = b3ToPos( (b3Vec3){ x, y, 0.0f } );
	...
	for ( int i = 0; i < pyramidHeight; ++i )
	{
		for ( int j = i / 2; j < pyramidHeight - ( i + 1 ) / 2; ++j )
		{
			for ( int k = i / 2; k < pyramidHeight - ( i + 1 ) / 2; ++k )
			{
				...
				bodyDef.position = b3ToPos( (b3Vec3){ x, y, z } );
...
static void CreateSmallPyramid( b3WorldId worldId, int baseCount, float extent, float centerX, float baseZ )
{
	...
	for ( int i = 0; i < baseCount; ++i )
	{
		...
		for ( int j = i; j < baseCount; ++j )
		{
			...
			bodyDef.position = b3ToPos( (b3Vec3){ x, y, baseZ } );
...
void CreateLargeWorld( b3WorldId worldId )
{
	...
	for ( int i = 0; i < gridCount; ++i )
	{
		...
		for ( int j = 0; j < gridCount; ++j )
		{
			...
			bodyDef.position = b3ToPos( (b3Vec3){ x, 0.0f, z } );
			b3BodyId body = b3CreateBody( worldId, &bodyDef );
...
void StepLargeWorld( b3WorldId worldId, int stepCount )
{
	...
	b3BodyDef bodyDef = b3DefaultBodyDef();
	bodyDef.type = b3_dynamicBody;
	bodyDef.position = b3ToPos( (b3Vec3){ x, 1.5f, z } );
...
void CreateWasher( b3WorldId worldId )
{
...

	{
		...
		b3BodyDef bodyDef = b3DefaultBodyDef();
		bodyDef.position = b3ToPos( (b3Vec3){ 0.0f, 21.0f, 0.0f } );
...
static void CreateTrees( b3WorldId worldId, int scale )
{
	...
	b3BodyDef bodyDef = b3DefaultBodyDef();
	bodyDef.position = b3ToPos( (b3Vec3){ 0.0f, 0.0f, 0.0f } );
	...
	{
		bodyDef.position = b3ToPos( (b3Vec3){ 0.0, 1.0f - z * yTilt, z } );
		b3BodyId bodyId = b3CreateBody( worldId, &bodyDef );
...
void CreateJunkyard( b3WorldId worldId )
{
	...
	b3BodyDef bodyDef = b3DefaultBodyDef();
	bodyDef.type = b3_kinematicBody;
	bodyDef.position = b3ToPos( (b3Vec3){ g_junkyardData.radius, 0.0f, 0.0f } );
